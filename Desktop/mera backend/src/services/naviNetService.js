  ...
  try {
    ...
    for (const row of data) {
      ...
      try {
        ...
        let customer = await Customer.findOne({ where: { MemberID } });

        // Check if the member exists in the PotentialCustomer table if not found in Customer
        let potentialCustomer = null;
        if (!customer) {
          potentialCustomer = await PotentialCustomer.findOne({ where: { MemberID } });
        }
        ...
        if (customer) {
          ...
          const hasDifferences = (
            ...
            potentialCustomer.Note !== (row.Note || null) ||
            potentialCustomer.CoordinatorID !== coordinator.CoordinatorID ||
            potentialCustomer.InsuranceID !== insurance.InsuranceID
          );

          if (hasDifferences) {
            console.log(`Differences found for potential customer ID: ${potentialCustomer.PotentialCustomerID}`);
            try {
              await potentialCustomer.update({
            ...
            // Optionally, log these changes in PendingChanges if needed
            for (const key in row) {
              if (row[key] !== potentialCustomer[key]) {
                try {
                  await PendingChanges.create({
                    PotentialCustomerID: potentialCustomer.PotentialCustomerID,
                    Field: key,
